    ...
    <?php if (mysqli_num_rows($result) > 0) { ?>
        <div class="table-responsive">
            <table class="table table-bordered text-center">
                ...
                <tbody>
                    <?php while ($pago = mysqli_fetch_assoc($result)) { ?>
                        <tr>
                            <td><?php echo $pago['id']; ?></td>
                            <td><?php echo date('d/m/Y H:i', strtotime($pago['fecha_pago'])); ?></td>
                            <td>$<?php echo number_format($pago['monto_total'], 0, ',', '.'); ?></td>
                            <td>
                                <a href="factura_pago.php?id=<?php echo $pago['id']; ?>" class="btn btn-success btn-sm">
...
<footer class="mt-5 py-4" style="background-color: #002244;">
    <div class="container text-center">
        <p class="mb-0 text-white small">© <?php echo date('Y'); ?> Pasaporte al Mundo - Todos los derechos reservados.
        </p>
    </div>
</footer>
